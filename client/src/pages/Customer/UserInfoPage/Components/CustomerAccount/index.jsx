...
import { deletePhotoRequestStart,
  updateUserRequestStart, } from '~/redux/auth/slice';
...
const CustomerAccount = () => {
  ...
  const [imageSource, setImageSource,] = React.useState(
    `${process.env.REACT_APP_HOST_IP}/${user?.avatar}` || null
  );
  ...
  const handleSavePublicInfo = () => {
    const form = new FormData();
    if (username !== user?.username) form.append('username', username);
    if (fullname !== user?.fullname) form.append('fullname', fullname);
    if (
      imageSource &&
      imageSource !== `${process.env.REACT_APP_HOST_IP}/${user?.avatar}`
    )
  ...
  const handleSaveContactInfo = () => {
    const form = new FormData();
    if (email !== user?.email) form.append('email', email);
    if (phone !== user?.phone) form.append('phone', phone);
    if (birthday !== user?.birthday) form.append('birthday', birthday);
    if (localAddress !== user?.address) form.append('address', localAddress);
  ...
  const render = () => (
    <div className='gap-3 flex flex-col'>
      <div className='rounded-xl p-3 bg-white'>
        ...
        <div>
          ...
          <div className='text-left'>
            <Button onClick={handleSavePublicInfo} variant='contained'>
              {translate('save')}
            </Button>
          </div>
        </div>
      ...
      <div className='rounded-xl p-3 bg-white'>
        ...
        <div className='flex flex-col gap-3 w-full'>
          ...
          <TextField
            ...
            size='small'
            placeholder={translate('address[0].addressDetail')}
      ...
      <div className='rounded-xl p-3 bg-white'>
        ...
        <QuiltedImageList
          itemData={user?.photos?.map((value) => ({
