    ...
    return (
      <div className='flex w-screen h-screen relative top-0 left-0'>
        <Paper className='w-1/3 h-fit p-5 pb-3 flex flex-col gap-3 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2'>
          ...
          }} onClick={handleGoogleLogin} variant='contained'><FontAwesomeIcon size='xl' className='mr-2' icon={faGoogle}/> {translate('login-with-google')}</Button>
          <div className='mt-3'>
            <span>{translate('dont-have-account')}? <Link to={authRoutes.logup}>{translate('sign-up')}</Link></span>
          </div>
        </Paper>
