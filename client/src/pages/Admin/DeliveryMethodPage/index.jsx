import React from 'react';
import { useDispatch, useSelector, } from 'react-redux';
import { ConfirmationModal, DataTable, Pagination, } from '~/components';
import { translate, } from '~/helpers';
import { createDeliveryMethodRequestStart,
  deleteDeliveryMethodRequestStart,
  getDeliveryMethodsRequestStart,
  updateDeliveryMethodRequestStart, } from '~/redux/deliveryMethod/slice';
import CreateDeliveryMethodModal from './components/CreateDeliveryMethodModal';
import UpdateDeliveryMethodModal from './components/UpdateDeliveryMethodModal';
import { Button, } from '@mui/material';

const DeliveryMethodPage = () => {
  const dispatch = useDispatch();
  const {
    deliveryMethods,
    meta,
    deliveryMethod,
    updateSuccess,
    deleteSuccess,
    createSuccess,
  } = useSelector((state) => state.deliveryMethod);

  const [orderBy, setOrderBy,] = React.useState('');
  const [descending, setDescending,] = React.useState(true);
  const [page, setPage,] = React.useState(1);
  const [limit, setLimit,] = React.useState(5);
  const [selectedObj, setSelectedObj,] = React.useState(null);

  const [showCreate, setShowCreate,] = React.useState(false);
  const [showUpdate, setShowUpdate,] = React.useState(false);
  const [showConfirm, setShowConfirm,] = React.useState(false);
  const [confirmAction, setConfirmAction,] = React.useState(() => () => {});
  const [confirmMessage, setConfirmMessage,] = React.useState('');

  const getDeliveryMethods = () => {
    dispatch(
      getDeliveryMethodsRequestStart({
        orderBy,
        page,
        limit,
        descending,
      })
    );
  };

  React.useEffect(() => {
    if (
      !deliveryMethods ||
      deliveryMethods.length === 0 ||
      deliveryMethods.length !== limit
    ) {
      console.log('fetch data');
      getDeliveryMethods();
    }
  }, [
    orderBy,
    descending,
    page,
    limit,
    dispatch,
    deliveryMethod,
    selectedObj,
    updateSuccess,
    deleteSuccess,
    createSuccess,
  ]);

  const updateDeliveryMethod = (id, deliveryMethodData) => {
    const data = {
      name: deliveryMethodData?.name,
      description: deliveryMethodData?.description,
      cost: deliveryMethodData?.cost,
    };

    dispatch(
      updateDeliveryMethodRequestStart({
        id,
        data: JSON.stringify(data),
      })
    );
  };

  const handleUpdate = (value) => {
    setSelectedObj(value);
    setShowUpdate(true);
  };

  const createDeliveryMethod = ({ name, description, cost, }) => {
    const data = {
      name,
      description,
      cost,
    };

    dispatch(createDeliveryMethodRequestStart(JSON.stringify(data)));

    setShowCreate(false);
  };

  const confirmUpdateDeliveryMethod = (id, deliveryMethodData) => {
    setConfirmAction(() => () => {
      updateDeliveryMethod(id, deliveryMethodData);
      setShowConfirm(false);
      setShowUpdate(false);
    });
    setConfirmMessage('Bạn có chắc chắn muốn lưu những thay đổi này không?');
    setShowConfirm(true);
  };

  const handleDelete = (value) => {
    setConfirmAction(() => () => {
      dispatch(deleteDeliveryMethodRequestStart(value?._id));
      setSelectedObj(null);
      setShowConfirm(false);
    });
    setConfirmMessage('Are you sure you want to delete this item?');
    setShowConfirm(true);
  };

  const render = () => (
    <>
      <ConfirmationModal
        body={confirmMessage}
        onConfirm={confirmAction}
        onHide={() => setShowConfirm(false)}
        title='Xác nhận'
        show={showConfirm}
      />
      <CreateDeliveryMethodModal
        setShow={setShowCreate}
        show={showCreate}
        createDeliveryMethod={createDeliveryMethod}
      />
      <UpdateDeliveryMethodModal
        setShow={setShowUpdate}
        show={showUpdate}
        updateDeliveryMethod={confirmUpdateDeliveryMethod}
        deliveryMethod={selectedObj}
      />
      <div className='flex flex-col'>
        <div className='leading-10 text-left py-2 mb-3 bg-gray-50 text-2xl'>
          {translate('delivery-method')}
        </div>
        <div className='flex flex-row w-full justify-between gap-3'>
          <div className='rounded-xl p-3 bg-white w-full'>
            <div className='text-right mb-3'>
              <Button onClick={() => setShowCreate(true)} variant='contained'>
                {translate('create')}
              </Button>
            </div>
            <DataTable
              actions={[
                {
                  label: translate('update'),
                  handler: handleUpdate,
                },
                {
                  label: translate('delete'),
                  handler: handleDelete,
                },
              ]}
              columns={[
                {
                  field: '_id',
                  enableSort: false,
                  label: translate('#'),
                },
                {
                  field: 'name',
                  enableSort: true,
                  label: translate('name'),
                },
                {
                  field: 'description',
                  enableSort: false,
                  label: translate('description'),
                },
                {
                  field: 'cost',
                  enableSort: false,
                  label: translate('cost'),
                },
              ]}
              data={deliveryMethods?.map((item) => ({
                ...item,
              }))}
              keyField='_id'
              onSort={(f, des) => {
                setOrderBy(f);
                setDescending(des === 'desc');
              }}
            />

            <Pagination
              count={meta?.totalPage}
              page={page}
              rowsPerPage={limit}
              setPage={setPage}
              setRowsPerPage={setLimit}
            />
          </div>
        </div>
      </div>
    </>
  );

  return render();
};

export default DeliveryMethodPage;
