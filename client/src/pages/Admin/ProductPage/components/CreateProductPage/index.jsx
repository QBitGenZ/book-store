...
import { createProductRequestStart, deleteImageRequestStart, } from '~/redux/product/slice';
...
const CreateProductPage = () => {
  ...
  const handleRemovePhoto = (image) => {
    dispatch(deleteImageRequestStart(image?.img?.split('/')[4]));
  };
  const handleAddImages = (imagesUpload) => {
    if (imagesUpload.length > 0) {
      ...
      setImagesSubmit(imagesUpload.map(image => image.file));
