...
const { getTypesRequestFailure, getTypesRequestSuccess, getTypesRequestStart, getTypesByAdminRequestSuccess, getTypesByAdminRequestFailure, getTypesByAdminRequestStart, getTypeRequestSuccess, getTypeRequestFailure, getTypeByAdminRequestSuccess, getTypeByAdminRequestFailure, getTypeRequestStart, getTypeByAdminRequestStart, createTypeByAdminRequestSuccess, createTypeByAdminRequestFailure, createTypeByAdminRequestStart, updateTypeByAdminRequestSuccess, updateTypeByAdminRequestFailure, updateTypeByAdminRequestStart, deleteTypeByAdminRequestSuccess, deleteTypeByAdminRequestFailure, deleteTypeByAdminRequestStart, } = require('./slice');
const { getAllRequestApi, getAllByAdminRequestApi, getOneRequestApi, getOneByAdminRequestApi, createOneByAdminApi, updateOneByAdminApi, deleteOneByAdminApi, } = require('./api');

function* handleGetTypesRequest(action) {
  try { const response = yield call(getAllRequestApi, action.payload);
    const { data, meta, } = response;
    yield put(getTypesRequestSuccess({
      data, meta,
    }));
...
function* handleGetTypesByAdminRequest(action) {
  try { 
    ...
    const { data, meta, } = response;
    yield put(getTypesByAdminRequestSuccess({
      data, meta, 
    }));
    yield put(showSnackbar({
      message: 'Request successful!', severity: 'success', 
...
function* handleGetTypeRequest(action) {
  try { const response = yield call(getOneRequestApi, action.payload);
    const { data, } = response;
...
export default function* watchTypesActions() {
  ...
  yield takeLatest(getTypesByAdminRequestStart.type, handleGetTypesByAdminRequest);
  yield takeLatest(getTypeRequestStart.type, handleGetTypeRequest);
  yield takeLatest(getTypeByAdminRequestStart.type, handleGetTypeByAdminRequest);
  yield takeLatest(createTypeByAdminRequestStart.type, handleCreateTypeByAdminRequest);
  yield takeLatest(updateTypeByAdminRequestStart.type, handleUpdateTypeByAdminRequest);
  yield takeLatest(deleteTypeByAdminRequestStart.type, handleDeleteTypeByAdminRequest);
}