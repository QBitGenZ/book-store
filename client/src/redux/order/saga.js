import { createOneRequestApi, getAllByAdminRequestApi, getAllRequestApi, updateOneRequestApi, } from '~/redux/order/api';

const { put, takeLatest, call, } = require('redux-saga/effects');
const { showSnackbar, } = require('../snackbar/slice');
const {
  getAllOrderRequestFailure, getAllOrderRequestSuccess, getAllOrderRequestStart,
  getAllByAdminRequestFailure, getAllByAdminRequestSuccess, getAllByAdminRequestStart,
  createOrderRequestFailure, createOrderRequestSuccess, createOrderRequestStart,
  updateOrderRequestFailure, updateOrderRequestSuccess, updateOrderRequestStart,
} = require('./slice');
...
function* handleGetAllOrderRequest(action) {
  try {
    ...
    yield put(getAllOrderRequestSuccess({
      data, meta,
    }));
    ...
    yield put(showSnackbar({
      message: `Order request failed: ${err.message}`, severity: 'error',
    }));
  }
}
...
function* handleGetAllByAdminRequest(action) {
  try {
    ...
    const { data, meta, } = response;
    yield put(getAllByAdminRequestSuccess({
      data, meta,
    }));
    yield put(showSnackbar({
      message: 'Admin order request successful!', severity: 'success',
    }));
  } catch (err) {
    yield put(getAllByAdminRequestFailure(err.message));
    yield put(showSnackbar({
      message: `Admin order request failed: ${err.message}`, severity: 'error',
    }));
  }
}
...
function* handleCreateOrderRequest(action) {
  try {
    ...
    yield put(showSnackbar({
      message: `Order creation failed: ${err.message}`, severity: 'error',
    }));
  }
}
...
function* handleUpdateOrderRequest(action) {
  try {
    const response = yield call(updateOneRequestApi, action.payload.id, action.payload.data);
    const { data, } = response;
    ...
    yield put(showSnackbar({
      message: `Order update failed: ${err.message}`, severity: 'error',
    }));
  }
}
