...
function* handleUpdateEventRequest(action) {
  try {
    const response = yield call(updateEventApi, action.payload.id, action.payload.data);
    const { data, } = response;
...
function* handleDonateBookRequest(action) {
  try {
    const response = yield call(donateBookApi, action.payload.eventId, action.payload.donation);
    const { data, } = response;
...
function* handleUpdateDonationStatusRequest(action) {
  try {
    const response = yield call(updateDonationStatusApi, action.payload.eventId, action.payload.donationId, action.payload.body);
    const { data, } = response;
...
export default function* watchEventActions() {
  ...
  yield takeLatest(getAllEventsByAdminRequestStart.type, handleGetAllEventsByAdminRequest);
  ...
  yield takeLatest(updateDonationStatusRequestStart.type, handleUpdateDonationStatusRequest);
  yield takeLatest(getEventStatisticsRequestStart.type, handleGetEventStatisticsRequest);
}
